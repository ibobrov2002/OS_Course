...
hpet_enable_interrupts_tim0(void) {
    ...
    hpetReg->TIM0_COMP = ticks_per_500ms & timer_val_mask;
...
hpet_enable_interrupts_tim1(void) {
    ...
    uint64_t ticks_per_1500ms = (3 * Peta / 2) / femptosec_per_tick;
...
pmtimer_cpu_frequency(void) {
    ...
    do {
        ...
        if (pm_start <= pm_cur) {
            ...
            pm_delta = pm_delta - pm_start + pm_cur;
        }
    } while (pm_delta < PM_FREQ / wait);
    // pm_delta / PM_FREQ - сколько секунд ждали, знаем с какой скоростью тикаем
