...
static uint32_t
_circular_shift(const uint32_t nbits, const uint32_t word) {
 ...
 */
int
sha1_reset(struct sha1* context) {
    if (context == 0) {
        return shaNull;
    }
 ...
 */
int
sha1_result(struct sha1* context, uint8_t Message_Digest[SHA1HashSize]) {
    int i;

    if ((context == 0) || (Message_Digest == 0)) {
        return shaNull;
    }
 ...
 */
int
sha1_input(struct sha1* context, const uint8_t* message_array, unsigned length) {
    if (length == 0) {
        return shaSuccess;
    }

    if ((context == 0) || (message_array == 0)) {
        return shaNull;
    }
    ...
    while ((length != 0) && (context->flags == 0)) {
...
//#define METHOD2
void
_process_block(struct sha1* context) {
    const uint32_t K[] = /* Constants defined in SHA-1 */
            {
                    0x5A827999,
                    0x6ED9EBA1,
 ...
 */
static void
_pad_block(struct sha1* context) {
    /*
