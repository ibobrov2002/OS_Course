...
writefile(struct Dir *dir, const char *name) {
    ...
    if (*name == 'o') {
        ...
        f->f_cred.fc_uid = 1;
        f->f_cred.fc_gid = 1;
    } else if (!strcmp(name, "fs/load/home/user")) {
        f->f_cred.fc_permission = S_IRUSR | S_IRGRP | S_IROTH;
...
void writedir(struct Dir *root, const char *full_name){
    ...
    jdir->f_cred.fc_permission = S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH | S_IWOTH;
...
main(int argc, char **argv) {
    ...
    root.f->f_cred.fc_permission = S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH | S_IWOTH;
